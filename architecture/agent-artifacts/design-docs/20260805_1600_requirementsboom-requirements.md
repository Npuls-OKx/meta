...
Aangescherpt na de eerste-indruk-review van 7 augustus (commentaar bij PR 131): rijen dragen verwijzings-id's, statuskolommen vervallen, de plaat blijft compact.
...
| R4 PoC-diepte | Drie epics zijn doorgewerkt tot stories: onderwijsaanbod specificeren en ontsluiten, student kiest onderwijsspecificaties, en aanbod plannen en roosteren. Voortgang en resultaat op leeruitkomsten krijgt een eerste opzet: features, en stories alleen waar de bron hard is. Overige epics blijven zonder verzonnen invulling. | `stories.md` bevat uitsluitend stories van deze vier epics; overige epics dragen hooguit benoemde features (geen stories), en een epic zonder features heeft in `features.md` één verklarende regel in plaats van een tabel. |
...
| R6 Overzicht boven volledigheid | De omvangslimieten uit de skill zijn hard: onder andere maximaal 8 epics (verruimd van 7 bij de boom-conceptreview van 6 augustus), 15 stories per epic, en de regellimieten per bestand. | `wc -l` per bestand binnen de limiet; tellingen epics en stories binnen het maximum. |
...
| R10 Achtergrondmechaniek buiten de boom | Geen eis-ID-traceernotatie of uitvoerbare scenario's (Gherkin) in de boomdocumenten; de index verwijst voor die mechaniek naar het ADR en #135. De leesbare verwijzings-id's per rij (E, F, S) zijn geen achtergrondmechaniek. | `grep` op `req~`, `Covers:` en `Functionaliteit:` in `architecture/docs/requirements/` levert niets op. |
...
| R12 Plaat en tabel vertellen hetzelfde | De mermaid-plaat in de index is een leesbare, met de hand klein gehouden weergave van opdracht, doelen en epics, binnen de plaatlimiet van de skill. | Elke epicknoop in de plaat staat in `epics.md` en omgekeerd, met id; de plaat bevat geen feature- of storyknopen en geen statusinformatie (aangepast na de eerste-indruk-review van 7 augustus). |
