...
> **Kopie per koppeling (ADR 0021).** Deze versie hoort bij de koppeling **OC-LMS**. Basis identiek aan de OC-P&R-versie; attributen kunnen per koppeling divergeren. **Eerste divergentie:** het object `leeruitkomst` is voor deze koppeling inhoudelijk uitgewerkt (titel, omschrijving, resultaat, gedrag), want dat is precies wat het LMS verder uitwerkt en aan de student exposet. Uitgewerkt voorbeeld: werkproces B1-K1-W1 in de payload.
...
- Verankering via leeruitkomst (vervangt `primaryCode`). Elke specificatie verwijst naar een leeruitkomst met een `type`.
- Divergentie OC-LMS: de leeruitkomst draagt hier ook de **inhoud** (titel, omschrijving, resultaat, gedrag uit het kwalificatiedossier). Voor de koppeling met planning volstaat de verwijzing; voor het LMS is de inhoud de kern. Voor nu op de korrel van het SBB-kwalificatiekader; fijnmazigere leeruitkomsten volgen later (ADR 0022). Nu de kwalificatiekader-typen (kwalificatiedossier, kwalificatie, kerntaak, werkproces, keuzedeel); later uitbreidbaar met een leeruitkomst-standaard zoals CompetentNL (ADR 0003, 0004).
- Id's zijn UUID's.
  ...
  "onderwijsspecificaties": [
    ...
    },
    {
      ...
      "leeruitkomst": {
        "type": "werkproces",
        "code": "B1-K1-W1",
        "titel": "Neemt de zorg-/adviesvraag in behandeling",
...
- Overige leeruitkomsten zijn in deze kopie nog niet inhoudelijk uitgewerkt (alleen B1-K1-W1 als voorbeeld); uitwerken per specificatie volgt.
