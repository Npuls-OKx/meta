...
| Koppeling | Een pijl op hoofdplaat v1.7 tussen twee componenten (de informatiestroom, conform ADR 0021). Heeft Public er een koppelingspecificatie voor, dan draagt de regel de koppeling-ID (OC-P&R, OC-SIS, OC-LMS); anders de markering "zonder koppelingspecificatie" |
...
- **Toestand.** Een objecttype ontstaat één keer; een latere verschijning van hetzelfde objecttype is een regel met soort `verandert` en een veld `toestand` uit een gesloten lijst met bron. Die lijst bevat nu alleen de stadia van aanbod uit het kaderscenario (planbaar, geroosterd). `Aanmelding` en `Inschrijving` zijn twee objecttypen op de plaat, verbonden door `wordt`; zij krijgen elk een ontstaat-regel met die relatie, geen toestandsovergang, zodat de vraag "subtype of toestand" open blijft voor de vragenpagina. Dekking telt op de eerste verschijning.
...
- **Pijlidentiteit.** Een pijl is de relatie-id uit Archi (`archimateRelationship` op de connectie) op de view "OKx hoofdplaat v1.7<concept> (zonder context applicaties)", met (van, naar) na het oplossen van junctions en de labelexpressie als leesbare naam; `stromen.json` uit de export is de enige bron. Die view is ook de view die gerenderd wordt (49 elementen, 43 connecties, 28 flows). Geen nummers, geen terugval op de tabel v20260317. Een stroomt-regel verwijst naar de relatie-id; waar (van, naar) op de view twee keer voorkomt (Planningssysteem naar Onderwijscatalogus, Roostersysteem naar SKS) onderscheidt de labelexpressie ze. "Geen pijl op de hoofdplaat" is een geldige lege regel die de controle rapporteert.
- **Koppeling-ID's.** In de leeswijzer een mappingregel: OC-P&R is OC naar Planningssysteem en terug; OC-SIS is OC naar KRS en OC naar SVS; OC-LMS is OC naar LMS. Andere pijlen: "zonder koppelingspecificatie".
- **Leslaag.** De modelleur heeft besloten de leslaag (Les specificatie, Lesgelegenheid, Lesgelegenheid verbintenis, Lesgelegenheid resultaat) mee te nemen. Dat is een herziening van ontwerpkeuze 8 en hoort met motivering in de eerstvolgende modelronde (feature 8, na 30 september). Tot die tijd draagt de regeltabel een scope-uitzondering voor die vier objecttypen met de motivering erbij: hoofdplaat v1.7 wisselt `Lesgelegenheid` uit van Roostersysteem naar KRS en het kaderscenario roostert tot lesniveau, dus de leslaag beweegt wel over een koppeling (besluit modelleur 17 september). De vragenpagina stelt de herziening ter toets: herkent de kerngroep die stroom.
- **Fase-indeling.** De fasenamen letterlijk uit de sectiekoppen "Fase 1" tot "Fase 8" van het kaderscenario; roosteren, de groepen en het inschrijven op gelegenheden horen in fase 4 ("Detailleren, roosteren en inschrijven"), niet in fase 2 of 3 (de mock-up wordt daarop gecorrigeerd); aanmelding, intake en plaatsing blijven fase 3. De verwachting welk objecttype in welke fase voor het eerst verschijnt komt uit de alinea "Wat licht op in de plaat" per fase en de laag-1-analyse, en wordt op 18 september vastgesteld.
...
| R6 Bijlage en deck | Zes pagina's: 1 de vraag aan de kerngroep en de legenda, 2 tot 4 een pagina per fase (chips plus regels; past fase 4 niet, dan alleen chips), 5 de plaat, 6 de vragen met invulblad. Het deck is dezelfde zes pagina's als slides, uit dezelfde SVG's | Pdf in `presentaties/export/` op 24 september op de branch; de modelleur en Garik (agenda-eigenaren) versturen uiterlijk 25 september bij de agenda die in het deck van 15 september al staat; doorloop van dertig minuten op 24 september |
...
Basis: PR 225 is op 18 september gemerged, zodat `informatiemodel.json`, `begrippen.json`, de generatorscripts, het publiceerscript en `tests/` op `dev` staan. Alle werk gebeurt in meta op één werkbranch `106-jochem-in-het-informatiemodel` vanaf `dev`, met per feature een commitreeks; één pull request naar `dev` na 30 september. Public volgt daarna, gestapeld op Public PR 104. Het model is tijdens de sprint bevroren: geen Archi-werk tot na 30 september; de ongecommitte layoutwijziging in de werkmap van de modelleur blijft buiten deze branch.
...
- **Tests volgens de testpersona.** `tests/test_<script>.py`, stdlib `unittest`, given-when-then, verwachtingen uit eigen fixtures. De tabellen per script in `tegenlezing-plan-tester.md` zijn op versie 1 geschreven; bij het aanmaken van sub-issue 1, 2, 3a en 3b worden ze bijgewerkt op versie 2 (relatie-id in plaats van pijlnummer, junction-resolutie, soort `verandert`, scope-uitzondering, witruimtenormalisatie, splitsing 3a en 3b) en dan letterlijk in de sub-issue gezet.
...
- **Werkafspraken.** Besluit van de modelleur op 18 september: één werkbranch in meta voor alle features (de sub-issues zijn de werkeenheden, elk met een herkenbare commitreeks), één pull request naar `dev` na 30 september met het agent-rapport; daarna Public. Dat wijkt af van "1 issue = 1 branch = 1 PR" en is bewust gekozen omdat de features elkaar dagelijks nodig hebben en het OKx-team één review wil doen.
- **Milestone.** Besluit van de modelleur: een eigen milestone "Voorbeelduitwerking Jochem, kerngroep 30 september" in meta en Public met einddatum 3 oktober, herleidbaar naar epic-0001 (feature-0001 en feature-0002); alle sub-issues eronder.
...
|---|---|---|---|
| do 17 sep | Plan versie 2 en tweede tegenlezing (gedaan) | agent | |
| vr 18 sep | Akkoord op het plan en de keuzes (gedaan: basis via PR 225, één werkbranch, eigen milestone, agenda bij modelleur en Garik); sub-issues en milestone; features 1, 2, 3a in uitvoering; pdf-export proefdraaien op een bestaand deck | agent; modelleur (één dagdeel) | Akkoord gegeven |
| ma 21 sep | Ochtend: fasenlijst, verwachting per fase, rollenlijst en testgevallen (asynchroon, half dagdeel); feature 4 fase 2, eerste render; feature 3b (optioneel) | agent; modelleur (half dagdeel) | Asynchroon via comment |
...
Capaciteit modelleur: vier van de zes dagdelen tot 25 september (18 september één, 21 september een half, 22 september één, 24 september één, 25 september een half); twee dagdelen reserve. Terugvalvolgorde bij uitloop: eerst feature 3b (JPG als plaat), dan fase 4 alleen als chips, dan de leslaag als grijze chips; nooit `stromen.json`, de vragenpagina, het invulblad of de verzenddatum.
...
| Modelleur valt uit op 25 september | Geen verzending | Garik is mede-agenda-eigenaar; bijlage staat op 24 september klaar op de branch |
| Agendaslot niet bevestigd | Geen tijd op 30 september | De agenda staat in het deck van 15 september; modelleur en Garik zijn eigenaar |
...
|---|---|---|---|---|---|---|
| 1 | Regeltabel, schema, fasenlijst en dekkingscontrole | meta | agent | 18 sep, akkoord kop 21 sep | basis | R1 en R2 tests groen; fasenlijst, verwachting per fase en rollenlijst akkoord (asynchroon, ochtend 21 sep) |
...
| 6 | Bijlage en deck kerngroep 30 september, invulblad | meta | agent; modelleur en Garik versturen | 24 sep, verzending 25 sep | 5 | R6: pdf en pptx in export, doorloop gedaan, bijlage verstuurd |
...
| 10 | Acceptatietest 30 september en uitkomst onder Public #106 | Public | modelleur, agent | 2 okt | 6 | R8: comment met per regel de antwoorden en per objecttype de termen, geanonimiseerd |
