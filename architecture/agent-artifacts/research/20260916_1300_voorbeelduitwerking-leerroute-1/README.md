...
Knikpunten schalen als absolute punten (Archi bewaart een knik als offset ten opzichte van bron en doel), zodat de pijlvorm van de modelleur behouden blijft; objectvakken krijgen per pijl kandidaatposities en het eerste vak dat niets raakt wint. De optie `--orthogonaal` vervangt schuine segmenten door rechte hoeken, maar levert op een plaat die niet voor rechte hoeken is getekend nieuwe kruisingen op; de routering van pijlen blijft daarom werk voor Archi (knikpunten of de Manhattan-router), het script reproduceert en verrijkt. Ter vergelijking: [met behoud van de pijlvorm](poc/hoofdplaat-v17-zonder-context-alleen-objecten.png) tegenover [orthogonaal](poc/hoofdplaat-v17-zonder-context-orthogonaal.png).
