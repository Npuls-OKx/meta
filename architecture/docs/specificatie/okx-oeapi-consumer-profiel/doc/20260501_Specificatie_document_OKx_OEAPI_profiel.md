...
7. **Bijsturen planning en aanbod**: Naarmate het schooljaar vordert wijkt de planning verder af van het initiële jaarplan. Studenten temporiseren of versnellen en er is uitval. De planner anticipeert en biedt de wijzigingen aan de roostermaker.  
...
**Student Keuze Systeem (SKS)** staat op deze plaat als duidelijk gescopetet **applicatiecomponent**: het faciliteert alleen het **keuzedeel-selectieproces** voor de student (voorkeurslijst, aanmelding keuzedeel, match op periode en locatie). **Oriëntatie en aanmelding op de opleiding** blijven via **Intake → KRS**; het SKS vervangt dat niet. Stromen op de plaat: **OC → SKS** levert `opleidingsprogramma-aanbod` (type keuzedeel); **Roostersysteem → SKS** levert roostercontext; **SKS → KRS** en **SKS → SVS** leggen `opleidingsprogramma-verbintenis` (keuzedeel) vast; **SKS → LMS** koppelt verbintenis en planninggroep voor uitvoering. Een volledig SKS (modulair kiezen, cross-instelling) valt buiten deze plaat — zie flexibelere leerroutes later in dit document.
...
| **Studiekeuzesysteem (SKS)** — *keuzedeel-variant* | **Keuzedeel-selectie** voor de student: geprioriteerde voorkeurslijst (periode, locatie), `aanmelding keuzedeel`, match op passend `opleidingsprogramma-aanbod` (type keuzedeel) en doorlevering naar registratie en uitvoering. | Geen volledig route- of modulesysteem; geen vervanging van **Intake** voor opleidingsaanmelding; geen bron voor `onderwijsspecificatie`, persoon of examenresultaat; geen tweede KRS. | `Aanmelding keuzedeel`; `opleidingsprogramma-verbintenis` (keuzedeel) richting **KRS** en **SVS**; planninggroep richting **LMS**. |
...
| **Studiekeuzesysteem (SKS)** — *keuzedeel-variant* | **Keuzedeel-keuzeprocesdossier** (voorkeurslijst, matchstatus; geen master voor persoon). | `Opleidingsprogramma-aanbod` (type keuzedeel) uit **OC**; keuzedeelruimte uit **Planningssysteem**; `persoon` en bestaande `opleidingsprogramma-verbintenis` uit **KRS** (read). | `Aanmelding keuzedeel`; doorlevering `opleidingsprogramma-verbintenis` (keuzedeel) naar **KRS** en **SVS**; koppeling planninggroep naar **LMS**. |
...
- **SKS vs Intake** — **Intake** doet opleidingsaanmelding en plaatsing; **SKS** (keuzedeel-variant) doet alleen keuzedeel-selectie na inschrijving. *Anti-patroon:* SKS wordt gebruikt voor eerste opleidingsinschrijving of SKS houdt een tweede persoonsregister. *Mitigatie:* SKS leest persoon en programmaplaatsing uit **KRS**; formele `opleidingsprogramma-verbintenis` (keuzedeel) schrijft SKS door naar **KRS**.
- **SKS vs OC** — OC is **publicatiekanaal** voor aanbod; SKS **consumeert** en kiest. *Anti-patroon:* SKS publiceert of muteert `onderwijsaanbod`. *Mitigatie:* SKS queryt **OC**; mutaties op aanbod blijven bij **Planning** / **Rooster** via **OC**.
